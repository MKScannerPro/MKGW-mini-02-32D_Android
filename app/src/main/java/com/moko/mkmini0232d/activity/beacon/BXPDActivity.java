...
import com.moko.mkmini0232d.databinding.ActivityBxpDInfoMini0232dBinding;
...
public class BXPDActivity extends BaseActivity<ActivityBxpDInfoMini0232dBinding> {
    ...
    @Override
    protected ActivityBxpDInfoMini0232dBinding getViewBinding() {
        return ActivityBxpDInfoMini0232dBinding.inflate(getLayoutInflater());
    }
    ...
    private void gotoAccParams() {
        if (isWindowLocked()) return;
        Intent intent = new Intent(this, BXPDAccParamsActivity.class);
        intent.putExtra(AppConstants.EXTRA_KEY_DEVICE, mMokoDevice);
        intent.putExtra(AppConstants.EXTRA_KEY_MAC, mBeaconInfo.mac);
        startActivity(intent);
    }
    ...
    private void gotoAdvParams() {
        if (isWindowLocked()) return;
        Intent intent = new Intent(this, BXPAdvParamsActivity.class);
        intent.putExtra(AppConstants.EXTRA_KEY_DEVICE, mMokoDevice);
        intent.putExtra(AppConstants.EXTRA_KEY_MAC, mBeaconInfo.mac);
        intent.putExtra(AppConstants.EXTRA_KEY_BEACON_TYPE, mBeaconInfo.type);
        startActivity(intent);
    ...
    private void powerOff() {
        ...
        dialog.setOnAlertConfirmListener(() -> {
            ...
            int msgId = MQTTConstants.CONFIG_MSG_ID_BLE_BXP_D_POWER_OFF;
    ...
    public void onMQTTMessageArrivedEvent(MQTTMessageArrivedEvent event) {
        ...
        if (msg_id == MQTTConstants.NOTIFY_MSG_ID_BLE_BXP_D_POWER_OFF) {
            //关机结果通知
