...
import com.moko.mkmini0232d.databinding.ActivityMkPirInfoMini0232dBinding;
...
public class MKPIRActivity extends BaseActivity<ActivityMkPirInfoMini0232dBinding> {
    ...
    @Override
    protected ActivityMkPirInfoMini0232dBinding getViewBinding() {
        return ActivityMkPirInfoMini0232dBinding.inflate(getLayoutInflater());
    }
    ...
    protected void onCreate() {
        ...
        mBind.tvRuntime.setText(String.format("%ds", mBeaconInfo.run_time));
        mBind.tvPowerOff.setOnClickListener(v -> powerOff());
        mBind.tvPIRSensorData.setOnClickListener(v -> gotoSensorData());
        mBind.tvPIRSensorParams.setOnClickListener(v -> gotoSensorParams());
    ...
    private void gotoSensorData() {
        ...
        Intent intent = new Intent(this, MKPIRSensorDataActivity.class);
        intent.putExtra(AppConstants.EXTRA_KEY_DEVICE, mMokoDevice);
        intent.putExtra(AppConstants.EXTRA_KEY_MAC, mBeaconInfo.mac);
        startActivity(intent);
    }
    ...
    private void gotoSensorParams() {
        if (isWindowLocked()) return;
        Intent intent = new Intent(this, MKPIRSensorParamsActivity.class);
        intent.putExtra(AppConstants.EXTRA_KEY_DEVICE, mMokoDevice);
        intent.putExtra(AppConstants.EXTRA_KEY_MAC, mBeaconInfo.mac);
        intent.putExtra(AppConstants.EXTRA_KEY_BEACON_TYPE, mBeaconInfo.type);
        startActivity(intent);
    ...
    private void gotoAdvParams() {
        if (isWindowLocked()) return;
        Intent intent = new Intent(this, MKAdvParamsActivity.class);
        intent.putExtra(AppConstants.EXTRA_KEY_DEVICE, mMokoDevice);
        intent.putExtra(AppConstants.EXTRA_KEY_MAC, mBeaconInfo.mac);
        intent.putExtra(AppConstants.EXTRA_KEY_BEACON_TYPE, mBeaconInfo.type);
        startActivity(intent);
    ...
    private void powerOff() {
        ...
        dialog.setOnAlertConfirmListener(() -> {
            ...
            int msgId = MQTTConstants.CONFIG_MSG_ID_BLE_MK_PIR_POWER_OFF;
    ...
    public void onMQTTMessageArrivedEvent(MQTTMessageArrivedEvent event) {
        ...
        if (msg_id == MQTTConstants.NOTIFY_MSG_ID_BLE_MK_PIR_POWER_OFF) {
            //关机结果通知
