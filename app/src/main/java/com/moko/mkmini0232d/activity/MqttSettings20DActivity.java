...
import com.moko.mkmini0232d.databinding.ActivityMqttDevice20dBinding;
...
import com.moko.mkmini0232d.fragment.SSLDevice20DFragment;
...
public class MqttSettings20DActivity extends BaseActivity<ActivityMqttDevice20dBinding> implements RadioGroup.OnCheckedChangeListener {
    private final String FILTER_ASCII = "[ -~]*";
    private GeneralDevice20DFragment generalFragment;
    private UserDevice20DFragment userFragment;
    private SSLDevice20DFragment sslFragment;
    ...
    @Override
    protected ActivityMqttDevice20dBinding getViewBinding() {
        return ActivityMqttDevice20dBinding.inflate(getLayoutInflater());
    }
    ...
    private void createFragment() {
        ...
        sslFragment = SSLDevice20DFragment.newInstance();
...
//                    row17.createCell(1).setCellValue("");
                ...
                runOnUiThread(() -> {
                    ...
                    if (isFileError) {
                        isFileError = false;
                        ToastUtils.showToast(MqttSettings20DActivity.this, "Export error!");
                        return;
                    }
                    ToastUtils.showToast(MqttSettings20DActivity.this, "Export success!");
                    Utils.sendEmail(MqttSettings20DActivity.this, "", "", "Settings for Device", "Choose Email Client", expertFile);
                    ...
                    new Thread(() -> {
                        try {
                            ...
                            if (rows < 18 || columns < 3) {
                                runOnUiThread(() -> {
                                    dismissLoadingProgressDialog();
                                    ToastUtils.showToast(MqttSettings20DActivity.this, "Please select the correct file!");
                                });
                                return;
                            }
                            ...
                            runOnUiThread(() -> {
                                dismissLoadingProgressDialog();
                                if (isFileError) {
                                    ToastUtils.showToast(MqttSettings20DActivity.this, "Import failed!");
                                    return;
                                }
                                ToastUtils.showToast(MqttSettings20DActivity.this, "Import success!");
                                initData();
                            });
                        } catch (Exception e) {
                            e.printStackTrace();
