...
import com.moko.mkmini0232d.databinding.ActivityBxpBCrInfoMini0232dBinding;
...
public class BXPBCRActivity extends BaseActivity<ActivityBxpBCrInfoMini0232dBinding> {
    ...
    private void getBatteryMode() {
        ...
        int msgId = MQTTConstants.CONFIG_MSG_ID_BLE_BXP_B_CR_BATTERY_MODE_READ;
    ...
    @Override
    protected ActivityBxpBCrInfoMini0232dBinding getViewBinding() {
        return ActivityBxpBCrInfoMini0232dBinding.inflate(getLayoutInflater());
    }
    ...
    public void onMQTTMessageArrivedEvent(MQTTMessageArrivedEvent event) {
        ...
        if (msg_id == MQTTConstants.NOTIFY_MSG_ID_BLE_BXP_B_CR_BATTERY_MODE_READ) {
            dismissLoadingProgressDialog();
        ...
        if (msg_id == MQTTConstants.NOTIFY_MSG_ID_BLE_BXP_B_CR_BATTERY_MODE_WRITE) {
            runOnUiThread(() -> {
                Type type = new TypeToken<MsgNotify<JsonObject>>() {
                }.getType();
                MsgNotify<JsonObject> result = new Gson().fromJson(message, type);
    ...
    private void setAdvMode(int value) {
        int msgId = MQTTConstants.CONFIG_MSG_ID_BLE_BXP_B_CR_BATTERY_MODE_WRITE;
        JsonObject jsonObject = new JsonObject();
        jsonObject.addProperty("mac", mBeaconInfo.mac);
