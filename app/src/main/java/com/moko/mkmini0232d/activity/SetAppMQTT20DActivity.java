...
import com.moko.mkmini0232d.databinding.ActivityMqttApp20dBinding;
import com.moko.mkmini0232d.dialog.AlertMessageDialog;
import com.moko.mkmini0232d.entity.MQTTConfig;
import com.moko.mkmini0232d.fragment.General20DFragment;
import com.moko.mkmini0232d.fragment.SSL20DFragment;
import com.moko.mkmini0232d.fragment.User20DFragment;
...
public class SetAppMQTT20DActivity extends BaseActivity<ActivityMqttApp20dBinding> implements RadioGroup.OnCheckedChangeListener {
    private final String FILTER_ASCII = "[ -~]*";
    private General20DFragment generalFragment;
    private User20DFragment userFragment;
    private SSL20DFragment sslFragment;
    ...
    @Override
    protected ActivityMqttApp20dBinding getViewBinding() {
        return ActivityMqttApp20dBinding.inflate(getLayoutInflater());
    }

    private void createFragment() {
        fragments = new ArrayList<>();
        generalFragment = General20DFragment.newInstance();
        userFragment = User20DFragment.newInstance();
        sslFragment = SSL20DFragment.newInstance();
    ...
    public void onMQTTConnectionCompleteEvent(MQTTConnectionCompleteEvent event) {
        ...
        runOnUiThread(() -> {
            ToastUtils.showToast(SetAppMQTT20DActivity.this, getString(R.string.success));
...
//                    row10.createCell(1).setCellValue("");
                ...
                runOnUiThread(() -> {
                    ...
                    if (isFileError) {
                        isFileError = false;
                        ToastUtils.showToast(SetAppMQTT20DActivity.this, "Export error!");
                        return;
                    }
                    ToastUtils.showToast(SetAppMQTT20DActivity.this, "Export success!");
                    Utils.sendEmail(SetAppMQTT20DActivity.this, "", "", "Settings for APP", "Choose Email Client", expertFile);
                    ...
                    new Thread(() -> {
                        try {
                            ...
                            if (rows < 13 || columns < 3) {
                                runOnUiThread(() -> {
                                    dismissLoadingProgressDialog();
                                    ToastUtils.showToast(SetAppMQTT20DActivity.this, "Please select the correct file!");
                                });
                                return;
                            }
                            ...
                            runOnUiThread(() -> {
                                dismissLoadingProgressDialog();
                                if (isFileError) {
                                    ToastUtils.showToast(SetAppMQTT20DActivity.this, "Import failed!");
                                    return;
                                }
                                ToastUtils.showToast(SetAppMQTT20DActivity.this, "Import success!");
                                initData();
                            });
                        } catch (Exception e) {
                            e.printStackTrace();
