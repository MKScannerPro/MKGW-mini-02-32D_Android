...
import com.moko.mkmini0232d.databinding.ActivityMqttDeviceModify20dBinding;
...
import com.moko.mkmini0232d.fragment.GeneralDevice20DFragment;
import com.moko.mkmini0232d.fragment.LWT20DFragment;
import com.moko.mkmini0232d.fragment.SSLDeviceUrl20DFragment;
import com.moko.mkmini0232d.fragment.UserDevice20DFragment;
...
public class ModifyMQTTSettings20DActivity extends BaseActivity<ActivityMqttDeviceModify20dBinding> implements RadioGroup.OnCheckedChangeListener {
    public static String TAG = ModifyMQTTSettings20DActivity.class.getSimpleName();
    private final String FILTER_ASCII = "[ -~]*";
    private GeneralDevice20DFragment generalFragment;
    private UserDevice20DFragment userFragment;
    private SSLDeviceUrl20DFragment sslFragment;
    private LWT20DFragment lwtFragment;
    ...
    @Override
    protected ActivityMqttDeviceModify20dBinding getViewBinding() {
        return ActivityMqttDeviceModify20dBinding.inflate(getLayoutInflater());
    }

    private void createFragment() {
        fragments = new ArrayList<>();
        generalFragment = GeneralDevice20DFragment.newInstance();
        userFragment = UserDevice20DFragment.newInstance();
        sslFragment = SSLDeviceUrl20DFragment.newInstance();
        lwtFragment = LWT20DFragment.newInstance();
...
//                    row17.createCell(1).setCellValue("");
                ...
                runOnUiThread(() -> {
                    ...
                    if (isFileError) {
                        isFileError = false;
                        ToastUtils.showToast(ModifyMQTTSettings20DActivity.this, "Export error!");
                        return;
                    }
                    ToastUtils.showToast(ModifyMQTTSettings20DActivity.this, "Export success!");
                    Utils.sendEmail(ModifyMQTTSettings20DActivity.this, "", "", "Settings for Device", "Choose Email Client", expertFile);
                    ...
                    new Thread(() -> {
                        try {
                            ...
                            if (rows < 18 || columns < 3) {
                                runOnUiThread(() -> {
                                    dismissLoadingProgressDialog();
                                    ToastUtils.showToast(ModifyMQTTSettings20DActivity.this, "Please select the correct file!");
                                });
                                return;
                            }
                            ...
                            runOnUiThread(() -> {
                                dismissLoadingProgressDialog();
                                if (isFileError) {
                                    ToastUtils.showToast(ModifyMQTTSettings20DActivity.this, "Import failed!");
                                    return;
                                }
                                ToastUtils.showToast(ModifyMQTTSettings20DActivity.this, "Import success!");
                                initData();
                            });
                        } catch (Exception e) {
                            e.printStackTrace();
